...
function ping(url?: string) {
  return new Promise((res, rej) => {
    new Client().connect(url).then(async client => {
...
command.setExecutor(async (app, msg, args) => {
  ...
  if (args[0] !== 'stable') pings.push(['beta', await ping('wss://towerverse-beta.herokuapp.com')])

  msg.channel.send(
    new MessageEmbed()
      .setTitle('TowerVerse Status')
      .addFields(pings.map(val => { return {name: val[0], value: `${val[1].toFixed()} ${typeof val[1] === 'number' ? 'ms' : ''}`} }))
  )
})
