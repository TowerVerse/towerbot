...
const defaultExecutor: Executor = (app, msg, args) => {
  return msg.reply("This command has no executor");
};
...
export class Command {
  ...
  async execute(client: App, message: Message): Promise<Command> {
    ...
    this._middlewareMethods.map((fn) =>
      proms.push(new Promise((res, rej) => fn(client, message, args, res)))
    );
